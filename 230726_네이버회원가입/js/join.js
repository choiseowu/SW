...
// input에 focousout 되면 부모 .inputbox border-act class remove
...
// 우측 아이콘 이미지를 원래대로 설정 (실행문2)

// .userpw input 값이랑 .user-chk input이 같은가? (조건2)
// pwchkveri에 true
// .warn에 뜨는 경고메세지 화면에서 사라지게 한다.
// 우측 아이콘 이미지를 바꿔준다.

// 그렇지 않다면 else (0이 아니고, 일치하지 않을 때)
// .userpw-chk .warn 비밀번호가 일치하지 않습니다. (text-red)
// 우측 아이콘 이미지를 원래대로 변경 
...
// .birth .warn에 text-red class로 '태어난 년도 4자리를 정확하게 입력하세요.'
...
$('#year, #month, #date').focusout(function(){
  ...
  // 1970년 1월 1일 00시 00분 00초 (UTC) 세계표준시를 기준으로 경과한 밀리초를 반환한다.
...
$('#veribtn').on('click', function(){
  ...
  // this의 형제요소인 div에 span태그로 불일치, disagree(x)
