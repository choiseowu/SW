...
// title.addEventListener('click', () => {
  ...
  // split - 구분자를 기준으로 문자열을 나눠 -> 배열을 만들어준다.
const chars = title.innerText.split('')
console.log(chars)

// 1. html에 있는  h1#title 안의 문자열 삭제
title.innerText = '';

// 2. span 태그에 담아서 title 안에 자바스크립트로 생성한 문자열 삽입
chars.forEach((char , index) => {
  // creatElement는 태그를 만들 때 쓰는 메소드 - span 태그를 만든다.
...
// setTimeout은 순차적으로 실행되는 것이 아니라 자신이 갖고있는 timer에 맞게 실행 타이밍을 가져간다.
// 비동기 함수
setTimeout(() => {console.log('첫번째 setTimeout')}, 500) 
setTimeout(() => {console.log('두번째 setTimeout')}, 800) 
setTimeout(() => {console.log('세번쨰 setTimeout')}, 100) 
