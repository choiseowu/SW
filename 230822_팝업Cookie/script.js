...
// setCookie, getCookie 두 가지 함수 생성 
// 1. setCookie : 쿠키 설정
...
function setCookie(name, value, hours){
  // 빈 문자열로 변수 초기화 = 쿠키 만료 시간 
  let expires = '';

  // 만약에 hours라는 값이 있다면 (true)
  if(hours) {
    // date라는 변수 안에 현재 시간을 가지는 
    // Date 객체를 생성 
    let date = new Date();
    ...
    // 시간을 다룰 때 일반적으로 밀리초(ms)로 계산한다.
    // Date 객체가 밀리초 단위로 시간을 나타내기 때문
    date.setTime(date.getTime() + (hours * 60 * 60 * 1000))

    // ; (세미콜론)
    // 쿠키 설정은 문자열 네임과 벨류로 구성되는데, 
    // 각각 네임과 값을 세미콜론으로 구분한다.
    // toUTCString() : Date 객체의 시간 => UTC형태의 문자열로 변환 
...
function getCookie(name){
  ...
  // 일치하는 쿠키를 찾지 못했을 떄, null 값을 반환해서 
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  closeBtn.addEventListener('click', function(){
    ...
    // closeBtn 클릭했을 때 nonePopup에 checked 속성이 있다면 setCookie라는 
    // 함수에 name, value, hours 라는 매개변수 값 세 개를 넘겨준다. 
