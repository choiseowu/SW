...
app.get('/info/:id', function(requests, response){
  ...
  // 데이터 찾을 때 requests.params.id String -> Int로 형 변환
...
// Session을 사용해서 로그인 기능 구현 ( 가장 많이 사용 )
...
passport.use(new LocalStrategy({
  ...
  db.collection('login').findOne({id : userID}, function(error, result){
    ...
    if(!result) {
      return done(null, false, {message : '없는 아이디 입니다.'})
    }
...
app.get('/mypage', getLogin ,function(requests, response){
  ...
  response.render('mypage.ejs', {info : requests.user.id})
})
